...
getattr(yfs_client::inum inum, struct stat &st)
{
	...
	// add for symbol link.
	yfs_client::syminfo syminf;
	ret = yfs->getsym(inum, syminf);
	if (ret != yfs_client::OK)	return ret;
	st.st_mode = S_IFLNK |0777;
	st.st_nlink = 1;
	st.st_atime = syminf.atime;
	st.st_mtime = syminf.mtime;
	st.st_ctime = syminf.ctime;
	st.st_size = syminf.size;
	printf("   getattr -> %llu", syminf.size);
    }
...
void fuseserver_symlink(fuse_req_t req, const char *buf, fuse_ino_t parent, const char *name)
{
	int r = -1;
	...
	fep.generation = 0;
	r = yfs->symlink(buf, parent, name, ino_out);

 	if (r == yfs_client::OK) {
		fep.ino = ino_out;
		getattr(ino_out, fep.attr);
