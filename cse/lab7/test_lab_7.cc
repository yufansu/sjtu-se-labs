...
main(int argc, char *argv[])
{
	...
	if ( (ret = writem(d1, "a", 64, '1')) != 0) {
		fprintf(stdout, "Place: 1\n");
		goto write_error;
	}

	if ( (ret = writem(d1, "r", 64, '1')) != EACCES) {
		fprintf(stdout, "Place: 2\n");
		goto write_error;
	}

	if ( (ret = writem(d1, "w", 64, '1')) != 0) {
		fprintf(stdout, "Place: 3\n");
		goto write_error;
	}
	if ( (ret = writem(d1, "n", 64, '1')) != EACCES) {
		fprintf(stdout, "Place: 4\n");
		goto write_error;
	}
...
reado_ok:
	...
	if ((ret=readm(d2, "ch")) != EACCES) {
		fprintf(stdout, "Place: 5\n");
		goto chmod_error;
	}
	if ((ret=readm(d3, "ch")) != EACCES) {
		fprintf(stdout, "Place: 6\n");
		goto chmod_error;
	}
	if ((ret=writem(d3, "ch", 64, '1')) != EACCES) {
		fprintf(stdout, "Place: 7\n");
		goto chmod_error;
	}
